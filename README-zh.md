...
这个插件是**常驻**的。在每个项目的每个会话里，PM 规则都会在会话开始时加载。它故意写得
很短——大约五十行。PM 的第一步是选一条"车道"：
...
只有 `team` 车道才会加载完整手册——一个叫 `crew:team-lane` 的 skill，大约四百行，包含
十四个步骤、文档格式、里程碑规则和状态文件格式。`ask` 会话永远不用为它付出代价。
...
| --- | --- | --- |
| PM 规则 | 一整段提示词，始终完整加载 | 常驻的只有精简版，完整手册放在 skill 里，走团队车道时才加载 |
