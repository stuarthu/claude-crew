...
// This module reads the job folder and produces one short notice, which
// `scripts/session-start.mjs` prints into the session — so an unfinished job is
// put in front of the PM instead of waiting to be remembered.
//
// Empty text contributes nothing, so a machine with no unfinished job pays
// nothing for this.

import { existsSync, readFileSync, readdirSync, statSync } from "node:fs";
import { join } from "node:path";

import { expandHome } from "./roles.mjs";
...
/** Default job folder; keep in step with the path named in the team-lane skill. */
export const DEFAULT_JOBS_DIR = "~/.claude/crew/jobs";
...
function readJob(dir) {
  ...
  // `JSON.parse` happily returns `null`, a number or an array. Anything that is
  // not a plain object is a state file nobody can act on, so report it as
  // unreadable rather than reading fields off it.
...
export function jobsNotice(jobsDir = DEFAULT_JOBS_DIR) {
  ...
  lines.push(
    "",
    "If one of these is in the folder this session is working in, load the `crew:team-lane` skill, then tell the user about it BEFORE anything else, and ask one question: carry on, or start clean. A job waiting for a milestone review is waiting for the user: ask that review question again before anything else moves. Never carry on without asking, and never start clean without asking. Ignore jobs that belong to another folder.",
  );
  return lines.join("\n");
}
