...
> **This file ships with the plugin**, so whoever is reading it may be in **any repository**. Three things follow, and they are worth saying first.
>
...
Every rule in `agents/*.md` is short and bossy on purpose — a role prompt is read
...
(`maxDepth: 1`), `preset/crew/agent.cordis.yml`, `skills/team-lane/SKILL.md`,
`agents/crew-architect.md`, `CLAUDE.md` design rule 1.
...
**Lives in** `agents/crew-architect.md`, `agents/crew-engineer.md`, `agents/crew-doc-reviewer.md`,
...
**Lives in** `agents/crew-architect.md` (names the tests), `agents/crew-engineer.md` (writes
them first), `agents/crew-code-reviewer.md` (blocks without them),
`agents/crew-doc-reviewer.md` (checks they are named).
...
**Lives in** `agents/crew-architect.md`, `skills/team-lane/SKILL.md`, `agents/crew-engineer.md`,
...
**Lives in** `skills/team-lane/SKILL.md` (step 4 **Write the opening document**, step 5
**Confirm**, step 8 **Design**, step 9 **Run the tasks**, step 12 **Milestone
review**), `agents/crew-architect.md`,
`agents/crew-doc-reviewer.md`, `host/jobs.js` (a restart says which milestone).
...
- **Solo.** One `crew-engineer` writes both halves of a task: the unit test
  first, then the code. This is the default, and nothing about it has changed.
- **Paired.** Two engineers each write one half. `crew-test-engineer` writes only
  the unit tests, `crew-code-engineer` writes only the product code, neither can
...
**Lives in** `agents/crew-engineer.md`, `agents/crew-test-engineer.md`,
`agents/crew-code-engineer.md`, `agents/crew-architect.md`, `skills/team-lane/SKILL.md`,
`agents/crew-code-reviewer.md`. Where that unit test file lives, and how it is run
...
**Lives in** `agents/crew-architect.md`, `agents/crew-code-reviewer.md` (reuse is a review
item).
...
**Lives in** `skills/team-lane/SKILL.md` (step 3 **Language and stack**), `agents/crew-researcher.md`,
...
**Lives in** `agents/crew-code-reviewer.md`, `agents/crew-engineer.md`.
...
not of will. In dsh-crew the line is `bash docs/qa/run-all.sh` at the end
...
**Lives in** `agents/crew-qa.md`, `agents/crew-engineer.md` ("Your test is a file that
stays"), `agents/crew-architect.md` (the test-file column in a task row),
`skills/team-lane/SKILL.md` (step 4 **Write the opening document**, step 10c **QA**, step 11
...
**Lives in** `skills/team-lane/SKILL.md` ("Documents are the only channel", "Change requests",
"An ADR quotes, it never points"),
`agents/crew-architect.md` ("When the PM sends you a CRD"), `agents/crew-engineer.md`,
`agents/crew-qa.md`.
...
PM asks a `crew-researcher` what those plans contain for this project type, with a
...
**Lives in** `skills/team-lane/SKILL.md` (step 12 **Milestone review** and step 13 **Release and
upgrade plans**), `agents/crew-researcher.md`.
...
(`~/.claude/crew/jobs/<job-slug>/state.json`) and into almost every git command of
...
**Lives in** `skills/team-lane/SKILL.md` (step 6 **Job folder**, step 7 **Branch**, step 17
...
**Lives in** `agents/crew-engineer.md` ("When you fix a bug: find at least two ways
first"), `agents/crew-architect.md` (**Your outputs**, decision records),
`agents/crew-doc-reviewer.md` (check 7), `skills/team-lane/SKILL.md` (step 10 **Check the finished
...
`skills/team-lane/SKILL.md` and `tools/verify-mount.mjs` were being rewritten by one task
...
bites. `agents/crew-engineer.md` ("A false red is not evidence") says it plainly: a red
...
**Lives in** `skills/team-lane/SKILL.md` (step 9 **Run the tasks** and step 10 **Check the
finished task**), `agents/crew-engineer.md` ("A false red is
not evidence").
...
- **Single-use, in the job folder** (`~/.claude/crew/jobs/<job-slug>/`, outside the
...
**Lives in** `skills/team-lane/SKILL.md` (step 4 **Write the opening document**, step 10c **QA**,
step 11 **Commit**, step 18 **Finish**, and the hard rules),
`agents/crew-qa.md` (the plan's home, and its step 6 **Feed the standing testability
list**),
`agents/crew-engineer.md`,
`agents/crew-architect.md`, `agents/crew-doc-reviewer.md`, `docs/qa/gaps.md`,
...
| team | Step 6, **Job folder** | PM | `state.json`: tasks, milestones, document versions, the CRD list, the merge result | `~/.claude/crew/jobs/<job-slug>/state.json` | **No, on purpose.** It is progress, not a record, and it stays out of the user's `git status` |
...
| big | Step 8, **Design** | `crew-architect` | the design, `docs/design/hld-<date>-<job-slug>.md`; `docs/design/tasks.md` with a **DoD section on every row**; one contract per boundary; an ADR per open choice, with every option and why it lost | `docs/design/`, `docs/design/api/<caller>-<callee>.md`, `docs/decisions/adr/` | Yes |
| big | Step 8, **Doc review before any code** — one round, like every other review | `crew-doc-reviewer` | findings, each blocking or optional — including "this row has no DoD section" | its report to the PM; the fix lands in the document | The report, no. The corrected documents, yes |
| team | Step 9, **Run the tasks** | PM starts one `crew-engineer` per task | the code and its test file, both named in the task row, with the failing run shown before the passing one | the project's own source and test folders | Yes |
...
| team | Step 15, **Last doc review** — one round | `crew-doc-reviewer` | findings on every document this job produced or changed, the README included | report to the PM; fixes land in the documents | The report, no. The documents, yes |
...
- **The researcher's answer had no home in the table.** Step 2 said "nothing of
  ...
  `agents/crew-researcher.md` has always written `docs/research/<short-name>.md`.
...
**Lives in** `skills/team-lane/SKILL.md` (**A bug becomes a task row, and you write its DoD
...
`agents/crew-architect.md` (**Task breakdown**), `agents/crew-engineer.md` (what to read
first, and the bug-fix section), `agents/crew-qa.md` (the plan starts from the task's
DoD section), `agents/crew-doc-reviewer.md` (check 1),
...
**Rule.** A task may be run in the **paired shape**. `crew-test-engineer` writes
only the unit test files; `crew-code-engineer` writes only the product code. Each
...
flat (principle 1), and a sibling is not a child, so `SendMessage` cannot reach
...
confusion: `crew-test-engineer` is a programmer, not a tester.
...
not opinions. First, this is not the last net: `crew-qa` (afterwards, writing its
...
**Lives in** `agents/crew-test-engineer.md`, `agents/crew-code-engineer.md`,
`agents/crew-engineer.md` (which says at the top that it is the solo road),
`skills/team-lane/SKILL.md` (the shape decision, the two worktrees, the merge, the one run, the
clean-up), `agents/crew-architect.md` (marking the shape in the task table, and the
interface ADR), `agents/crew-code-reviewer.md` (the evidence it must be handed, and the
...
**Why (ours).** Three pieces of evidence, all of them from dsh-crew's own
...
**Lives in** `skills/team-lane/SKILL.md`, step 2 of the team lane. That step is the whole
...
One requirement is dsh-crew's own and comes from the user rather than from
...
| --- | --- | --- | --- |
| **unit test** | One behaviour per test, written before the code that satisfies it exists | `crew-engineer` in the solo shape, `crew-test-engineer` in the paired shape | The project's own test suite; a file the task owns, committed with the code |
| **case** (a QA case) | Acceptance, black box: one DoD item checked the way the user would see it, after the code is finished | `crew-qa` | **Only** `docs/qa/<task-id>/`, with a `run.sh` per task |
| **the project's test command** | In dsh-crew `npm test`: it runs both of the above and every other check together | — | `package.json`, `scripts.test` |
| **contract test** | One on each side of a module boundary, proving that side matches the boundary contract (principle 3) | `crew-engineer`, or the engineers of a paired task | The project's own test suite (this repository has no module boundary today, so it has none) |
...
13 (every test lands on disk), 21 (the paired shape) — as well as `agents/crew-qa.md`.
...
prompts carries them **word for word**: `skills/team-lane/SKILL.md`, `agents/crew-architect.md`, `agents/crew-engineer.md`,
`agents/crew-test-engineer.md`, `agents/crew-code-engineer.md`, `agents/crew-qa.md`, `agents/crew-code-reviewer.md`,
`agents/crew-security-reviewer.md`, `agents/crew-doc-reviewer.md` and `agents/crew-researcher.md`.
...
**Why (ours).** Nothing in dsh-crew said a word about this until the `apply-req` job, and
...
in `skills/team-lane/SKILL.md` say the same thing; if they ever disagree, this one is the source and the other is
the copy.
...
The last two rows settle a disagreement this file used to contain. `skills/team-lane/SKILL.md` step 14 called all
...
- When you change a rule in `agents/*.md`, change the principle here that carries
